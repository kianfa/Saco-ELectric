...
export type AddToCartProduct = Omit<CartItem, "lineId" | "quantity"> & { lineId?: string }
...
function createLineId(productId: string, selectedVariantId: string | null | undefined) {
  return selectedVariantId ? `${productId}:${selectedVariantId}` : productId
}
...
function parseStoredCart(value: string | null): CartItem[] {
  ...
  try {
    ...
    return parsed
      .filter((item): item is CartItem => Boolean(item && typeof item.productId === "string" && typeof item.slug === "string" && typeof item.name === "string" && typeof item.price === "number" && typeof item.quantity === "number"))
      .map((item) => {
        const selectedVariantId = typeof item.selectedVariantId === "string" && item.selectedVariantId ? item.selectedVariantId : null
        return {
          ...item,
          lineId: typeof item.lineId === "string" && item.lineId ? item.lineId : createLineId(item.productId, selectedVariantId),
          selectedVariantId,
          selectedVariantLabel: typeof item.selectedVariantLabel === "string" && item.selectedVariantLabel ? item.selectedVariantLabel : null,
          quantity: Math.min(normalizeQuantity(item.quantity), getMaxAllowedQuantity(Number(item.stockQuantity) || 0)),
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = useCallback((product: AddToCartProduct, quantity = 1) => {
    ...
    const lineId = product.lineId ?? createLineId(product.productId, product.selectedVariantId)
    ...
    setItems((currentItems) => {
      ...
      if (safeQuantity < nextQuantity) stockLimited = true
      if (existingItem) {
        if (safeQuantity === existingItem.quantity) return currentItems
        added = true
        return currentItems.map((item) => item.lineId === lineId ? { ...item, ...product, lineId, quantity: safeQuantity } : item)
      }
      if (safeQuantity <= 0) return currentItems
      added = true
      return [...currentItems, { ...product, lineId, quantity: safeQuantity }]
    })
    if (stockLimited) showStockWarning()
    if (added) toast.success("محصول به سبد خرید اضافه شد")
    return added
  }, [])

  const removeFromCart = useCallback((lineId: string) => setItems((currentItems) => currentItems.filter((item) => item.lineId !== lineId)), [])
  ...
  const updateQuantity = useCallback((lineId: string, quantity: number) => {
    ...
    setItems((currentItems) => currentItems.map((item) => {
      ...
      if (requestedQuantity > maxQuantity) showStockWarning()
  ...
  const decreaseQuantity = useCallback((lineId: string) => {
    const item = items.find((cartItem) => cartItem.lineId === lineId)
    if (!item) return
    if (item.quantity <= 1) return removeFromCart(lineId)
  ...
  const value = useMemo<CartContextValue>(() => ({ items, isHydrated, addToCart, removeFromCart, increaseQuantity, decreaseQuantity, updateQuantity, clearCart, getCartTotal, getCartItemsCount, totals }), [addToCart, clearCart, decreaseQuantity, getCartItemsCount, getCartTotal, increaseQuantity, isHydrated, items, removeFromCart, totals, updateQuantity])
